...
var TwitchChat = (function () {
  ...
  function update(channel, user, message, self) {
    ...
    if(TwitchClient.opts.channels.length > 1 && showChannel) chatLine.appendChild(chatChannel);
  ...
  function unhost(channel, viewers) { 
    hosting(channel, null, viewers, true); 
  }

  function connecting(address, port) {
  ...
  function logon() {
    if(showConnectionNotices) {
      chatNotice('Authenticating', 1000, -3, 'chat-connection-good-logon');
    }
  }

  function connectfail() {
    if(showConnectionNotices) {
      chatNotice('Connection failed', 1000, 3, 'chat-connection-bad-fail');
    }
  }

  function connected(address, port) {
    if(showConnectionNotices) {
      chatNotice('Connected', 1000, -2, 'chat-connection-good-connected');
    }
    joinAccounced = [];
  }
  ...
  function disconnected(reason) {
    if(showConnectionNotices) {
      chatNotice('Disconnected: ' + (reason || ''), 3000, 2, 'chat-connection-bad-disconnected');
    }
  }

  function reconnect() {
    if(showConnectionNotices) {
      chatNotice('Reconnected', 1000, 'chat-connection-good-reconnect');
    }
  }
